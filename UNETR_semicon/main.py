...
parser.add_argument('--logdir', default='testtest111', type=str, help='directory to save the tensorboard logs')   #####remember change the filename to _1000
...
#logic
    w = torch.tensor([0.2809,4.5012,4.7226,174.1168]).cuda(0)
# #memory
#     w = torch.tensor([0.2095,8.5922,12.1042,1200.7941,37.3878]).cuda(0)
    ...
    post_pred = AsDiscrete(argmax=True,
                           to_onehot=True,
                           n_classes=args.out_channels)
    dice_acc = DiceMetric(include_background=True,           ########################modify True or False
                          reduction=MetricReduction.MEAN,
                          get_not_nans=True)
    model_inferer = partial(sliding_window_inference,  #Partial functions allow us to fix a certain number of arguments of a function and generate a new function.
                            roi_size=inf_size,
