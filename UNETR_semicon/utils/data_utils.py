...
def get_loader(args):
    ...
    train_transform = transforms.Compose(
        ...
        transforms.ScaleIntensityRanged(keys=["image"],
                                            ...
                                            b_min=args.b_min,
                                            b_max=args.b_max,
                                            clip=True),###if no this intensity normalization, loss will be nan

        transforms.RandRotated(keys=["image", "label"],range_x = rotation_x,range_y = rotation_y,range_z = rotation_z,prob=0.2),
        transforms.RandScaleIntensityd(keys="image",factors=0.1, prob=0.2),
        transforms.Rand3DElasticd(keys=["image", "label"],sigma_range = (9.,13.),magnitude_range=(0.,900.),prob=0.2),
        transforms.RandAdjustContrastd(keys=["image","label"],prob=0.3,gamma=(0.7,1.5)),
        transforms.RandFlipd(keys=["image", "label"],
                                 prob=args.RandFlipd_prob,
                                 spatial_axis=0),
        transforms.RandFlipd(keys=["image", "label"],
